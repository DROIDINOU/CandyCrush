...
int main()
{
    ...
    sound = LoadSound("../assets/hope.mp3");
    music = LoadMusicStream("../assets/hope.mp3");
    ...
    while (!WindowShouldClose() && niveau < 1)
    {
        ...
        if (attenteClics)
        {
            if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON))
            {
                ...
                // Calcul des positions tel que dans afficher_grille
                int tailleCase = 50;
    ...
    CloseAudioDevice();
    // Fermeture de l'audio et de la fenêtre
    CloseWindow();
