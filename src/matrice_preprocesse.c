...
int ObtenirReponseAuMessage(const char message[][3][100], int index);
...
# 20 "matrice.c"
const char MESSAGEETREPONSESATTENDUES[4][3][100] = {
...
void initialiser_grille(GrilleBonbons *grille) {
    ...
    for (int i = 0; i < grille->lignes; i++) {
        for (int j = 0; j < grille->colonnes; j++) {
            grille->tableau[i][j].pion = couleurs[rand() % 5];
            grille->tableau[i][j].gelatine = 
# 38 "matrice.c" 3 4
                                            0
# 38 "matrice.c"
                                                 ;
...
# 50 "matrice.c"
    ...
    while (nombreGelatine > 0) {
        ...
        if (!grille->tableau[x][y].gelatine) {
            grille->tableau[x][y].pion = 'G';

            grille->tableau[x][y].gelatine = 
# 60 "matrice.c" 3 4
                                            1
# 60 "matrice.c"
                                                ;
...
void afficher_grille(GrilleBonbons *grille) {
    for (int i = 0; i < grille->lignes; i++) {
        for (int j = 0; j < grille->colonnes; j++) {
            printf(" %c ", grille->tableau[i][j].pion);
        }
        printf("\n");
    }
}
...
int ObtenirReponseAuMessage(const char message[][3][100], int index) {
    ...
    do {
        printf("%s", message[index][0]);
        int result = scanf(" %d", &reponse);
        while (getchar() != '\n');
...
# 127 "matrice.c"
    ...
    i = x2 + 1;
    while (i < 20 && grille->tableau[i][y2].pion == pion) {
        compteur++;
        xFin = i;
        i++;
    }
...
# 153 "matrice.c"
    ...
    j = y2 + 1;
    while (j < 20 && grille->tableau[x2][j].pion == pion) {
        compteur++;
        yFin = j;
        j++;
    }
...
# 181 "matrice.c"
    ...
    i = x1 + 1;
    while (i < 20 && grille->tableau[i][y1].pion == pion) {
        compteur++;
        xFin = i;
        i++;
    }
...
# 212 "matrice.c"
    ...
    j = y1 + 1;
    while (j < 20 && grille->tableau[x1][j].pion == pion) {
        compteur++;
        yFin = j;
        j++;
    }
...
# 274 "matrice.c"
    ...
    maGrilleGelatine.lignes = 20;
    maGrilleGelatine.colonnes = 20;
